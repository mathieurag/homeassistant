...
class myClientEnedis:
    ...
    def readDataJson(self):
        ...
        log.debug(f"fichier lu directory : {directory}")
        listeFile = glob.glob(directory)
        log.debug(f"fichier lu listeFile : {listeFile}")
    ...
    def manageLastCallJson(self):
        ...
        if lastCallInformation is not None:
            try:
                ...
                log.debug(f"manageLastCallJson : lastUpdate : {lastUpdate}")
                ...
                log.debug(f"manageLastCallJson : statutLastCall : {statutLastCall}")
                if statutLastCall is not None:
                    self.updateStatusLastCall(statutLastCall)
                version = lastCallInformation.get("version", None)
                log.debug(f"manageLastCallJson : previous version : {version}")
            except:
    ...
    def writeDataJson(self):
        ...
        for clef in self.getDataJsonKeys():
            try:
                ...
                log.debug(f" >>>> ecriture : {nomfichier} / {data}")
                if not nePasEcrire:
    ...
    def getData(self) -> bool:
        ...
        if self.contract.isLoaded:
            ...
            log.debug(f"UpdateRealise : {self.getUpdateRealise()}")
    ...
    def getDelayIsGoodAfterError(self, currentDateTime):
        ...
        if timeLastCall is not None:
            ...
            ecartOk = True
            log.debug("DelayIsGoodAfterError: TimeLastCall is None, True")
        # test
        # ecartOk = True
        return ecartOk
    ...
    def getHorairePossible(self):
        ...
        log.debug(f"HorairePossible: {hourMin}<={hourNow}<2330 => {horairePossible}")
    ...
    def getCallPossible(self, trace=False):
        ...
        level = logging.ERROR if trace else logging.DEBUG
        log.log(
            level,
    ...
    def callConsommation(self):
        if self.isConsommation():
            ...
            log.info("Mise à jour consommation OK")
    ...
    def callProduction(self):
        if self.isProduction():
            ...
            self.updateTimeLastCall()
            self.updateStatusLastCall(True)
            log.info("mise à jour production OK")
    ...
    def callEcoWatt(self):
        ...
        self.updateTimeLastCall()
        self.updateStatusLastCall(True)
        log.info("mise à jour EcoWatt OK")
    ...
    def callTempo(self):
        ...
        self.updateTimeLastCall()
        self.updateStatusLastCall(True)
        log.info("mise à jour Tempo OK")

    def update(self):  # noqa C901
        log.debug(f"myEnedis ...new update ?? {self._PDL_ID}")
        if self.contract.isLoaded:
            if self.getCallPossible():
                try:
                    log.debug(
