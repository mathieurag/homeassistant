...
class myDataEnedisProduction:
    ...
    def updateData(
        ...
        if onLance:
            ...
            log.debug(f"--updateData {clefFunction} ( du {dateDeb} au {dateFin} )--")
            self._data = data
            if self._data is None:
                ...
                callDone = True
            log.debug("updateData : data %s", (self._data))
            ...
            log.debug(f"with update !! {clefFunction} ( du {dateDeb} au {dateFin} )--")
            log.debug("updateData : data %s", (self._data))
        else:
            log.debug(f"noupdate !! {clefFunction} ( du {dateDeb} au {dateFin} )--")
            log.debug("no updateData : data %s", (self._data))
        return self._data
