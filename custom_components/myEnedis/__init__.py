...
async def async_setup(hass: HomeAssistant, config: ConfigType) -> bool:
    ...
    for enedis_conf in conf:
        ch = dir(enedis_conf)
        _LOGGER.debug(f"enedis_conf data {ch}")
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if heurescreuses == "":
        heurescreuses = None
    _LOGGER.debug(f"**enedis**_conf heurescreuses *{heurescreuses}*")
...
class sensorEnedisCoordinator(DataUpdateCoordinator):
    ...
    def update_data(self) -> bool:
        ...
        _LOGGER.info(f"Initialisation du PDL {self._PDL_ID}")
    ...
    async def async_set_options(self):
        ...
        _LOGGER.debug(f"async_set_options - proc -- {self.entry.options}")
        if not self.entry.options:
            ...
            _LOGGER.debug(".config_entry.options() - done")
        _LOGGER.debug("async_set_options - proc -- done ")

    def update_OptionsMyEnedis(self):
        _LOGGER.debug(
            "update_MyEnedis pre-getini for {}".format(self.entry.options["token"])
        )
        _LOGGER.debug("getInit()")
        ...
        try:
            ...
            if not os.path.isdir(path):
                _LOGGER.debug(f"creation repertoire  ? {path}")
                os.mkdir(path)
                _LOGGER.debug(f"repertoire cree {path}")
        except:
            _LOGGER.error("error")
        ...
        _LOGGER.debug(f"fichier lu {len(dataJson)}")
...
async def options_updated_listener(hass, entry):
    ...
    _LOGGER.debug("options_updated_listener ")
    _LOGGER.debug("options_updated_listener - done -- ")
