...
class myCall:
    ...
    def post_and_get_json(self, serviceEnedis=None, params=None,
        ...
        while maxTriesToGo > 0:
            ...
            try:
                ...
                self.setLastAnswer(dataAnswer)
                _LOGGER.debug(f"{logPrefix}headers : {headers} =====")
                _LOGGER.debug(f"{logPrefix}data : {data} =====")
                _LOGGER.debug(f"{logPrefix}reponse : {dataAnswer} =====")
                maxTriesToGo = 0  # Done
                hasError = False
    ...
    def getDataPeriod(self, deb: str, fin: str | None) -> tuple[str, bool]:
        if fin is not None:
            _LOGGER.debug(f"--get dataPeriod : {deb} => {fin} --")
            payload = {
