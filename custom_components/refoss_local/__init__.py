...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    discover = await refoss_local_discovery_server(hass)
    refoss_local_discovery = DiscoveryService(hass, entry, discover)
    hass.data[DOMAIN][DATA_DISCOVERY_SERVICE] = refoss_local_discovery

    await hass.config_entries.async_forward_entry_setups(entry, PLATFORMS)

    async def _async_scan_update(_=None):
        await refoss_local_discovery.discovery.broadcast_msg()
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if hass.data[DOMAIN].get(DATA_DISCOVERY_SERVICE) is not None:
        refoss_local_discovery: DiscoveryService = hass.data[DOMAIN][DATA_DISCOVERY_SERVICE]
        refoss_local_discovery.discovery.clean_up()
